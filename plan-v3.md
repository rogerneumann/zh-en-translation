...
### M4: Extended Language Support (Partial)
...
3. ✅ **User glossary management**:
   - CSV-based glossary file at `get_config_path().parent / "glossary.csv"`
   - `engines/glossary.py`: `load_glossary()` / `save_glossary()` with robust error handling
   - `pipeline.translate()` accepts optional `glossary` dict; applies as override before dictionary lookup
   - Glossary tab in Preferences: table editor with Add/Remove row, Import CSV, Export CSV
   - Glossary auto-loaded on Preferences open; saved on OK/Apply
4. **Pinyin romanization options**:
   - Toggle between Hanyu Pinyin, Wade-Giles, Pinyin (with/without tones)

**Status:** M4.3 complete ✅ (branch `v3-m4-glossary`). M4.1, M4.2, M4.4 deferred to v3.2+
