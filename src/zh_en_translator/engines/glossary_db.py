...
class GlossaryDB:
    ...
    def load(self, domain: str) -> dict[str, str]:
        ...
        logger.debug("Loaded %d terms for domain '%s' from %s", len(result), domain, self.db_path)
        return result
    ...
    def add_term(
        ...
        logger.debug("add_term: domain=%s zh=%s -> %s (id=%d)", domain, chinese, english, row_id)
        return row_id
