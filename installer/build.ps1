...
if (-not $SkipVersionBump) {
    ...
    Write-Ok "Committed and pushed v${NewVersion}"
} else {
    Write-Step "Step 1.6: Skipping version bump (-SkipVersionBump)"
    # Read the current version for informational output
    $VerLine = (Get-Content (Join-Path $RepoRoot "src\zh_en_translator\__init__.py") |
                Where-Object { $_ -match "^__version__" } | Select-Object -First 1)
    Write-Ok "Current version: ${VerLine}"
}
